        ...
        switch(data[1]){
            ...
            case "STATUS":
                if (Array.isArray(data[3])) {
                    if (data[3][0] == "UIDNEXT") {
                        this._selectedMailbox.UIDNext = data[3][1];
                    }
                }
                return;
            case "OK":
...
IMAPClient.prototype.openMailbox = function(path, options, callback){
    ...
    // Courier-IMAP doesn't return UIDNEXT on SELECT/EXAMINE, so get status
    this._send("STATUS " + this._escapeString(path) + " (UIDNEXT)", (function(status){}));
